...
(**
   A map where
    - keys are classes gref and
    - values are maps of instances gref to their implementation
*)
let instances_of_classes : instances_of_classes ref = Summary.ref GlobRef.Map.empty ~name:"instances_of_classes"

(**
  A map where
    - keys are instances gref and
    - values are instances implemtations
*)
let instances_map : instances ref = Summary.ref GlobRef.Map.empty ~name:"instances_map"

let get_instance_from_gref (g : GlobRef.t) : instance =
  GlobRef.Map.get g !instances_map
...
let add_instance (inst : instance) insts =
  ...
  instances_map := GlobRef.Map.add inst.is_impl inst !instances_map
...
let remove_instance inst =
  instances_map := GlobRef.Map.remove inst.is_impl !instances_map;
  let insts =
    try GlobRef.Map.find inst.is_class !instances_of_classes
...
let is_instance gr =
  GlobRef.Map.mem gr !instances_map
